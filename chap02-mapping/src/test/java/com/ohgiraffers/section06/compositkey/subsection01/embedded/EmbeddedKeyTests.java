...
public class EmbeddedKeyTests {
    ...
    * 이 방법은 복합키를 구성하는 필드들을 별도의 클래스로 분리한 뒤, 해당 클래스를 @IdClass 어노테이션의 값으로 지정해 주는 것이다.
    ...
    * 이는 jpa에서 엔티티 객체의 동일성응ㄹ 판단하기 위해 필요하다.
    * 또한, @GeneratedValue 어노테이션을 사용하여 복합 키를 자동으로 생성하는 것은 불가능하다는 점에 주의해야 한다.
    *
    * 두 방식 모두 복합키 클래슨느 영속성 컨텍스트가 관리하지 않느다는 특징이 있으며, 큰 기능적 차이도 존재하지 않는다.
