...
class ProblemsOfUsingDirectSQLTests {
    ...
    void closeConnection() throws SQLException {
        con.rollback(); // 테스트 끝난 후 롤백 - 데이터베이스에 테스트용 입력값이 쓸데없이 추가되는걸 막기 위해 전부 없앰
        con.close();
    }
    ...
    * 2. SQL에 의존하여 개발
    * 3. 패러다임의 불일치(상속, 연관 관계, 객체 그래프 탐색)
    * 4. 동일성 보장 문제
    * */
    //1. 데이터 변환, SQL 작성, JDBC API 코드 등의 중복 작성(개발 시간 증가, 유지보수성 저하)
    ...
    void testDirectInsertSql() throws SQLException {
        ...
        pstmt.setString(1,menu.getMenuName()); // 1 <- 물음표 순서
    ...
    * 이러한 수정 영향을 미치는 것은 오류를 발생시킬 가능성도 있지만 유지보수성에도 악역향을 미친다.
    ...
    * 데이터베이스 테이블에 맞춘 객체 모델
    ...
    void testEquals() throws SQLException {
        ...
        Assertions.assertEquals(menu, menu2); // 주소값 달라서 false but JPA에서는 true 가 나온다 -> 연속성 컨텍스트
    }
