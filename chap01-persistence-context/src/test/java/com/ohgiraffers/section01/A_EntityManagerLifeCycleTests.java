...
public class A_EntityManagerLifeCycleTests {
    ...
    /*
    * 엔티티 매니저 팩토리(EntityManagerFactory)란? db랑 커넥션 맺은 객체
    * 엔티티 매니저를 생성할 수 있는 기능을 제공하는 팩토리 클래스이다.
    * Thread-safe(한번에 접근하는 것을 막기위함)하기 때문에 여러 스레드가 동시에 접근해도 안전하므로 서로 다른 스레드 간 공유해서 재사용한다.
    ...
    * */
    ...
    /*
    * 엔티티 매니저(Entity Manager)란? 메모리상에서 관리되는 데이터베이스, db 조작을 위한 인터페이스
    ...
    * */
    ...
    public void 엔터티_매니저_팩토리와_엔터티_매니저_생명주기_확인1(){
        ...
        System.out.println("entityManager.hashCodle : " + entityManager.hashCode());
    }
